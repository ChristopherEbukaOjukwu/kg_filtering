...
Targets in data:
# Initial Data Collection from Open Targets

Three association datasets and one target metadata dataset were downloaded from the Open Targets Platform on **May 20, 2026**.

**Source page:** <https://platform.opentargets.org/downloads/association_by_datasource_direct/access>  
...
This dataset provides **source-specific evidence** for gene–disease associations.

**Used for:**

- **F2:** source-type constraints
- **F4:** metapath/type constraints

```bash
wget --recursive --no-parent --no-host-directories --cut-dirs 6 \
ftp://ftp.ebi.ac.uk/pub/databases/opentargets/platform/26.03/output/association_by_datasource_direct .
```
...
This dataset provides **evidence-type scores** for gene–disease associations.
...
```bash
wget --recursive --no-parent --no-host-directories --cut-dirs 6 \
ftp://ftp.ebi.ac.uk/pub/databases/opentargets/platform/26.03/output/association_by_datatype_direct .
```
...
This dataset provides the main **gene–disease edge score**.

**Used for:**

- **F1:** retaining associations with scores greater than or equal to a chosen threshold.

```bash
wget --recursive --no-parent --no-host-directories --cut-dirs 6 \
ftp://ftp.ebi.ac.uk/pub/databases/opentargets/platform/26.03/output/association_overall_direct .
```
...
This dataset provides metadata for target nodes.

It is the node-level analogue of the overall association file: one row per **Ensembl gene ID**, with canonical metadata such as:
...
```bash
wget --recursive --no-parent --no-host-directories --cut-dirs 6 \
ftp://ftp.ebi.ac.uk/pub/databases/opentargets/platform/26.03/output/target .
```
