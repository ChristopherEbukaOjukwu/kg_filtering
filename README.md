...
### H1: Attention-associated retention

Do individual filters retain genes with higher publication attention, even though publication count is not directly used as a filter?

Publication attention is measured as:
...
Do stacked filters retain a gene–disease set that differs from what would be expected from the individual filters alone?

This tests whether filters act independently or interact.

---

### H3: Under-survival relative to attention

Given a gene’s publication attention, do some genes survive less often than expected?

This identifies genes that are highly published but still fail to survive the full filter stack.
...
| Component | Status |
|---|---|
| Open Targets data collection | complete |
| Gene2PubMed publication counts | complete |
| Filter rationale | complete |
| Falsification criteria | complete |
| PPI add-on | complete |
| Canonical input table | complete |
...
This project does not claim that filtered-out genes are biologically unimportant.

The analysis focuses on how filtering choices change graph composition and downstream eligibility.

The main claim is methodological:

> Common filtering decisions systematically reshape which genes and gene–disease pairs remain available for downstream analysis.
